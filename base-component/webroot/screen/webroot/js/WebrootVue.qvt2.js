...
- render Vue template client side
*/
...
moqui.webrootVue = Vue.createApp({
    ...
    data: function () {
        return {
            ...
            // current path as a list of path elements (strings), does not include extraPathList;
            //     updated by subscreens-active when path changes
            currentPathList: [],
            // list of extra path elements (strings), these are passed to the last screen in the path
            extraPathList: [],
            // current search string (computed)
            // body parameters (for POST requests)
            bodyParameters: {}, currentParameters: {},
            // list of NavMenuItem objects, see methods below for structure
            navMenuList: [],
            // list of active subscreen components (Vue component instances)
            activeSubscreens: [],
            // user and system info
            userId: "", username: "", locale: "en",
            ...
            navHistoryList: [], navPlugins: [], accountPlugins: [], notifyHistoryList: [],
            activeContainers: {}, urlListeners: [],
            moquiSessionToken: "", appHost: "", appRootPath: "",
            sessionTokenBc: null, qzVue: null, leftOpen: false, moqui: moqui
        }
    },
    methods: {
...
moqui.webrootVue.component('m-subscreens-active', {
    ...
    methods: {
        loadActive: function () {
            ...
            if (urlInfo.search && urlInfo.search.length > 0) {
                qvt2FullPath += '?' + urlInfo.search;
            }
            root.loading++;
...
// Use global router from routes.js if available, otherwise create one
let appRouter;
try {
    if (typeof router !== 'undefined') appRouter = router;
} catch (e) { }

if (!appRouter) {
    appRouter = VueRouter.createRouter({
        history: VueRouter.createWebHistory(),
        routes: [
            { path: '/:pathMatch(.*)*', component: moqui.EmptyComponent }
        ]
    });
}

moqui.webrootVue.use(appRouter);
// Expose router for manual usage/debugging
moqui.webrootVue.$router = appRouter;
moqui.webrootRouter = appRouter;

// Sync router changes to WebrootVue state
appRouter.afterEach((to) => {
    if (moqui.webrootVue && moqui.webrootVue.setUrl) {
        // console.log('Router navigation to: ' + to.fullPath);
        // Update WebrootVue state without pushing to history (router already did)
        // Check if path is different to avoid redundant updates? 
        // setUrl handles some checks, but let's just pass it through.
        moqui.webrootVue.setUrl(to.fullPath, null, null, false);
    }
});
